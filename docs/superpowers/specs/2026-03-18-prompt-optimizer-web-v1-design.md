...
| 设计令牌 | 亮色模式 | 暗色模式 |
|---------|---------|---------|
| 页面背景 | `#FAFAF9`（暖白） | `#0C0A09`（暖黑） |
| 表面色（编辑器、卡片） | `#FFFFFF`（白） | `#1C1917`（暖灰-900） |
| 主要文字 | `#1C1917`（深暖黑） | `#FAFAF9`（暖白） |
| 强调色 | `#10B981`（翡翠绿） | `#34D399`（浅翡翠绿） |
| 边框 / 分割线 | `#E7E5E4`（stone-200） | `#44403C`（stone-700） |
| 次要文字 | `#78716C`（stone-500） | `#A8A29E`（stone-400） |

- 强调色翡翠绿在两种模式下色相一致，暗色模式使用更亮的变体以保证对比度
- 灰色体系使用 Tailwind stone 色阶（暖灰），与翡翠绿形成温暖自然的搭配
