...
| 设计令牌 | 亮色模式 | 暗色模式 |
|---------|---------|---------|
| 页面背景 | `#F8FFFE`（薄荷白） | `#0F172A`（墨蓝黑） |
| 表面色（编辑器、卡片） | `#FFFFFF`（白） | `#1E293B`（slate-800） |
| 主要文字 | `#1E293B`（深墨蓝） | `#F1F5F9`（slate-100） |
| 强调色 | `#0D9488`（深薄荷） | `#2DD4BF`（亮薄荷） |
| 辅助色 | `#E11D48`（玫瑰红） | `#FB7185`（浅玫瑰） |
| 边框 / 分割线 | `#E2E8F0`（slate-200） | `#334155`（slate-700） |
| 次要文字 | `#64748B`（slate-500） | `#94A3B8`（slate-400） |

- 强调色薄荷绿用于大部分交互元素（按钮、tab、链接）
- 辅助色玫瑰红仅用于**重要/醒目操作**（如「采纳」按钮、错误状态），形成冷暖碰撞的视觉焦点
- 灰色体系使用 Tailwind slate 色阶（冷蓝灰），与薄荷绿形成清冷一致的基调
...
- 按钮：主要操作用薄荷强调色填充，高优先级操作（「采纳」）用玫瑰辅助色填充，次要操作用描边样式
- Tab：活跃 tab 使用薄荷强调色下划线
- 卡片（诊断）：细边框，表面色背景
